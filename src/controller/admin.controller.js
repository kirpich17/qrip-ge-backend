...
exports.adminSignin = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(401).json({ status: false, message: 'Invalid credentials or access denied' });
    }
    ...
    if (user.userType !== 'admin') {
      return res.status(403).json({ status: false, message: 'Forbidden: You do not have administrator privileges.' });
    }
